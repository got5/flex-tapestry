...
Used to save and share the value of a variable in controllers during the user navigation. For example, if a variable *currentUser* with *Persist* metadata is in a controller A, its value will be saved when the user will quit the view related to controller A. If controller B wants to get the value of *currentUser* variable set in controller A, it will just have to define a variable also named *currentUser*, with the same *Persist* metadata. And when the user will come back to the view linked to controller A, *currentUser* variable will be initialized with its last value.

This is the annotation to use if you want to share a persistent data among your controllers, like a shopping cart, a logged user, ...
